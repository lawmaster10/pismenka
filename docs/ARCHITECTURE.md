...
| `rescue` | After a wrong answer on a letter or number target | Two-tier queue: wrong non-rescue target → easy retry after **one** intervening round (`dueAfterRounds: 1`); failed easy rescue → mid retry in **2–3** rounds (`midRescueDelayRange`). Rescue is a round override, not a lasting `phase` value. Number rescues force a 4-option grid, `.avoid` distractors, and (easy tier) a known-numbers pool. The difficulty governor may shorten a number rescue timer only to one round, never zero, and dequeue rejects the immediately previous target as a second guard. |
...
**Numbers sessions share this engine skeleton.** When `plan.primaryLayer == .numbers`, the same `warmup` / `drill` / `plainReview` / `rescue` phases run, but rounds are built by `AdaptiveGameState.buildNumberRound(...)`: targets come from the introduced number pool (warm-up by `numbersByReviewPriority`, drill by the active spotlight/focus with the same focus-chance, hello-focus deadline, and per-day ask-cap machinery, review-test days by `chooseWeeklyNumberAssessmentTarget`), distractors come from `NumberDifficulty.pickDistractors` under the frozen `NumberInstructionalBand`'s confusion policy (relaxed tier by tier before ever shrinking the grid), and the typed round carries `target: .number(...)` / `activityKind: .numberRecognition` so `recordAnswer` writes `numberStats` and `RoundEvent.unitKind == .number`. Ordinary introduction challenges pass every warm-up, focus, review, and rescue choice through a final fail-closed gate: the target must differ from the immediately previous target and remain below `Profile.numberDailyTargetAskLimit` (**5 asks per local day**). If no introduced/focus target satisfies both invariants, the sitting ends rather than exceeding the cap. Same-day recency is seeded from persisted number `RoundEvent`s and checkpointed in `GameEngineSnapshot`; Extra Practice is exempt from this intentionally but does not write the daily fairness ledger. The letter-only `maintenance` and `contrast` phases and cameo letters have no numbers equivalents today.
...
| `dailyTargetAskDay`, `dailyTargetAskCounts` | Per-letter target-ask counts for the current local day; seeds each new sitting's engine counts so the fail-closed 5-ask-per-letter introduction-day cap holds per calendar day. Extra Practice is excluded; the numbers twin has the same contract. |
...
| Per-letter target asks | **Accumulate across same-day sessions** through `dailyTargetAskDay` / `dailyTargetAskCounts`; each new sitting seeds `AdaptiveGameState.sessionTargetCounts` from them (merged per-letter-max with any restored checkpoint), so the fail-closed introduction-day 5-ask cap holds per calendar day |
...
Repetition is intentionally tuned down compared with the older focus-heavy loop. Focus-target chance is lower in scaffolded/normal modes, recent targets are restored across same-day sittings and checkpoints, needs-work targets are weighted-sampled rather than always taking the top-scored unit, no letter or number may be targeted more than 5 times per local day on introduction days, and no ordinary daily target may equal the immediately previous target. Rescue retries retain at least one intervening round. The shared final assignment invariant is fail-closed: when every introduced target is capped or would repeat immediately, the sitting ends instead of bypassing the rule. Extra Practice is intentionally repetitive but isolated from both daily fairness ledgers.
...
| `dailyTargetAskDay`, `dailyTargetAskCounts` | `LocalDay?`, `[String: Int]` | Ordinary Letters-challenge fairness ledger; seeds the fail-closed 5-ask-per-letter local-day cap, clamps legacy over-limit values, and excludes Extra Practice/calibration/progress-check asks |
...
| `numberDailyTargetAskDay`, `numberDailyTargetAskCounts` | `LocalDay?`, `[String: Int]` | Ordinary Numbers-challenge fairness ledger; seeds the fail-closed 5-ask-per-number local-day cap, clamps legacy over-limit values, and excludes Extra Practice/calibration/progress-check asks |
