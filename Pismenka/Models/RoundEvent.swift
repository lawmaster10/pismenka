...
struct RoundEvent: Codable, Equatable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // Lossy optional enums: unknown future cases (e.g. a new UnitKind)
        // must decode as nil, not fail the whole RoundEvent / Profile.
        unitKind = try c.decodeLossyIfPresent(UnitKind.self, forKey: .unitKind)
        activityKind = try c.decodeLossyIfPresent(LearningActivityKind.self, forKey: .activityKind)
        ...
        mistakeType = try c.decodeLossyIfPresent(MistakeType.self, forKey: .mistakeType)
        ...
        liveDifficulty = try c.decodeLossyIfPresent(LiveDifficulty.self, forKey: .liveDifficulty)
        isRescue = try c.decode(Bool.self, forKey: .isRescue)
        rescueDifficulty = try c.decodeLossyIfPresent(RescueDifficulty.self, forKey: .rescueDifficulty)
        attemptContext = try c.decodeLossyIfPresent(AttemptContext.self, forKey: .attemptContext)
        ...
        planReason = try c.decodeLossyDecodableIfPresent(RoundPlanReason.self, forKey: .planReason)
    }
